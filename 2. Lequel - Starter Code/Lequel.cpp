...
TrigramProfile buildTrigramProfile(const Text &text)
{
    ...
    for(const auto& line : text) {                                   
        ...
        for(int n = 0; (n + 3) <= unicodeString.length(); n++) {    //the loop is terminated once the iterator points to
            ...
            trigramList[trigram]++;                                 //each time the trigram is repeated, its value increases. 
                                                                    //That's how each trigram's frequency is calculated.
            trigramKey.clear();
            trigram.clear();
...
float getCosineSimilarity(TrigramProfile &textProfile, TrigramProfile &languageProfile)
{
    ...
    for(auto& pair : textProfile) {                         //we did not opt for pair to point to each element of languageProfile
